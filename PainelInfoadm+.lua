...
script_version("8.9.41-AutoUpdate")
...
local state = {
    ...
    esp_active_check = imgui.ImBool(false),
    esp_dist_val = imgui.ImFloat(6000.0),
    spectate_id = -1,
...
local function check_update()
    ...
    sampAddChatMessage("[PI] Verificando atualizacoes...", 0xFFFF00)
    local temp_path = os.getenv("TEMP") .. "\\paineladm_update.tmp"
...
if cfg.main.esp_active == nil then cfg.main.esp_active = false end
if not cfg.main.esp_dist then cfg.main.esp_dist = 6000.0 end
...
state.esp_active_check.v = cfg.main.esp_active
state.esp_dist_val.v = cfg.main.esp_dist
...
-- IP Extractor vars
local stop_spec_requested = false
...
prof_type_map["ladrao de veiculos"] = "Mafia"; prof_type_map["chefao da mafia"] = "Mafia"

local esp_weapon_names = {
...
function sampev.onServerMessage(color, text)
    ...
    if state.device_scanner_active and state.current_scan_info and state.current_scan_info.name then
        ...
        local ip_match = text:match("(%d+%.%d+%.%d+%.[%d%*]+)")
        ...
        if txt:find(name_lower, 1, true) or txt:find("ip:") or txt:find("%d+%.%d+%.%d+%.[%d%*]+") or txt:find("android") or txt:find("mobile") or txt:find("celular") or txt:find("launcher") then
            ...
            elseif txt:find("%d+%.%d+%.%d+%.[%d%*]+") then
                if not state.player_devices[state.current_scan_info.id] then detected = "PC" end
            end

            if detected then 
                state.player_devices[state.current_scan_info.id] = {type = detected, ip = ip_match}
...
function sampev.onSendCommand(cmd)
    ...
    if c:find("^/pararespiar") or c:find("^/specoff") or c:find("^/reoff") then
        stop_spec_requested = true
        -- Reseta a flag apos 3 segundos caso o servidor nao responda ou algo falhe
        lua_thread.create(function() wait(3000) stop_spec_requested = false end)
    end
end

function sampev.onSpectatePlayer(playerId, camType)
    state.spectate_id = playerId
    state.spectate_vehicle_id = -1
end

function sampev.onSpectateVehicle(vehicleId, camType)
    state.spectate_vehicle_id = vehicleId
    state.spectate_id = -1
end

function sampev.onTogglePlayerSpectating(toggle)
    if toggle then
        stop_spec_requested = false -- Garante que a flag esteja desligada ao entrar no spec
    else
        state.spectate_id = -1; state.spectate_vehicle_id = -1
        if stop_spec_requested then
...
function sampev.onShowDialog(id, style, title, button1, button2, text)
    if state.device_scanner_active then
        ...
        if state.current_scan_info and detected then
            state.player_devices[state.current_scan_info.id] = {type = detected, ip = nil}
        end
...
function sampev.onDisplayGameText(style, time, text) if state.device_scanner_active then return false end end
...
function sampev.onShowTextDraw(id, data)
    if state.device_scanner_active then
        if data and data.text then
            ...
            local detected = nil
            if content:find("android") or content:find("mobile") or content:find("celular") or content:find("ios") then detected = "Mobile" elseif content:find("pc") or content:find("computador") or content:find("windows") or content:find("launcher") or content:find("client") or content:find("desktop") then detected = "PC" end
            if state.current_scan_info and detected then state.player_devices[state.current_scan_info.id] = {type = detected, ip = nil}; state.scan_response_received = true end
        end
        return false 
    end
end
...
local function draw_config_tab()
    ...
    if imgui.Checkbox("Ativar Wallhack (ESP)", state.esp_active_check) then
        cfg.main.esp_active = state.esp_active_check.v
        inicfg.save(cfg, "PainelInfo_Config_v8.ini")
    end
    if state.esp_active_check.v then
        if imgui.SliderFloat("Distancia ESP", state.esp_dist_val, 50.0, 10000.0, "%.0f m") then
            cfg.main.esp_dist = state.esp_dist_val.v
            inicfg.save(cfg, "PainelInfo_Config_v8.ini")
        end
    end
...
local function draw_players_tab()
    local is_nov=(state.active_tab==1); local is_on=(state.active_tab==2); local show_pcol=is_on; local hdr=is_nov and "Novatos:" or is_on and "Online:" or ""
    imgui.Text(hdr)
    if not is_nov then
        imgui.SameLine()
        imgui.PushItemWidth(150)
    ...
    if is_nov then imgui.TextDisabled("Lista de jogadores novatos (Nivel < 12). Botao direito no nome para opcoes.") else imgui.TextDisabled("Lista de todos os jogadores online. Staff no topo. Botao direito para opcoes.") end
    
    local search_u8 = u8(state.search_text.v):lower(); local search_cp = string.lower(state.search_text.v)
    local data={}; local staff={}; local players={}; local cnt=0; local maxid=sampGetMaxPlayerId()
    ...
    for i=0,maxid do 
        if sampIsPlayerConnected(i) then 
            ...
            if nick and #nick>0 then 
                ...
                local ping=sampGetPlayerPing(i) or 0; local score=sampGetPlayerScore(i) or 0; local color; local prof; 
                if rank then color=get_rank_color(rank); prof=get_display_rank(rank) else local argb=sampGetPlayerColor(i); color=convert_samp_color(argb); prof=get_closest_profession_name(argb) end; 
                local pdata={id=i,nick=nick,rank=rank,color=color,is_staff=rank~=nil,Level=score,Ping=ping,profession=prof}; 
                
                if not rank then local p_type = prof_type_map[prof:lower()]; if p_type == "Mafia" then total_mafia = total_mafia + 1 else total_honest = total_honest + 1 end end
                local dev = state.player_devices[i]; if dev == "PC" then total_pc = total_pc + 1 elseif dev == "Mobile" then total_mobile = total_mobile + 1 end
                local dev_data = state.player_devices[i]
                if dev_data then
                    if dev_data.type == "PC" then total_pc = total_pc + 1 elseif dev_data.type == "Mobile" then total_mobile = total_mobile + 1 end
                end
                ...
                if not is_nov then
                    if filter_sel == "PC" then if dev ~= "PC" then filter_pass = false end
                    elseif filter_sel == "Mobile" then if dev ~= "Mobile" then filter_pass = false end
                    if filter_sel == "PC" then if not dev_data or dev_data.type ~= "PC" then filter_pass = false end
                    elseif filter_sel == "Mobile" then if not dev_data or dev_data.type ~= "Mobile" then filter_pass = false end
                    elseif filter_sel == "Mafia" then if prof_type_map[prof:lower()] ~= "Mafia" then filter_pass = false end
                    elseif filter_sel == "Honestas" then if prof_type_map[prof:lower()] ~= "Legal" then filter_pass = false end end
                end

                local passes=((is_on) or (is_nov and pdata.Level<12 and not paused)) and filter_pass
                if passes then 
                    local prof_u8=u8(pdata.profession or ""):lower(); local nick_u8=u8(nick):lower(); local id_s=tostring(i); 
                    local m_txt=nick_u8:find(search_u8,1,true) or prof_u8:find(search_u8,1,true); local m_id=id_s:find(search_cp,1,true); 
                    if m_txt or m_id then table.insert(data,pdata); if is_on then if pdata.is_staff then table.insert(staff,pdata) else table.insert(players,pdata) end else table.insert(players,pdata) end end; cnt=cnt+1 
                end 
            end 
        end 
    end
    
    if is_nov then imgui.Text(string.format("Total: %d",#data)) else
        imgui.Text(string.format("Total: %d (Staff: %d | Players: %d)", #staff+#players, #staff, #players))
        imgui.SameLine(); imgui.TextColored(IMAGE_GREY, "|"); imgui.SameLine(); imgui.TextColored(IMAGE_GREEN, string.format("Honestas: %d", total_honest)); imgui.SameLine(); imgui.TextColored(IMAGE_RED, string.format("Mafia: %d", total_mafia))
        imgui.SameLine(); imgui.TextColored(IMAGE_GREY, "|"); imgui.SameLine(); imgui.TextColored(IMAGE_BLUE, string.format("PC: %d", total_pc)); imgui.SameLine(); imgui.TextColored(IMAGE_PINK, string.format("Mobile: %d", total_mobile))
        imgui.SameLine()
        if not state.device_scanner_active then
            if imgui.Button("Escanear Dispositivos") then
                lua_thread.create(function()
                    state.device_scanner_active = true; state.player_devices = {}; sampAddChatMessage("[PI] Iniciando scan de dispositivos (/ip)...", 0x00FF00)
                        ...
                        if sampIsPlayerConnected(i) then
                            ...
                            if nick then
                                for attempt = 1, 2 do
                                    ...
                                    state.current_scan_info = {id = i, name = nick}; state.scan_response_received = false; state.scan_message_count = 0
                                    sampSendChat("/ip " .. i); state.device_scan_progress = i
                                    local timeout = 0
                                    while not state.scan_response_received and timeout < 300 do wait(10); timeout = timeout + 10; if state.scan_message_count > 0 and timeout > 100 then break end end
                    ...
                    state.current_scan_info = nil; state.device_scanner_active = false; sampAddChatMessage("[PI] Scan de dispositivos concluido!", 0x00FF00)
                end)
            end
        else
    ...
    local function render_plist(list,show_pc)
        ...
        for _,p in ipairs(list) do
            ...
            local p4s=p4ls+lvlw; imgui.SameLine(p4s); imgui.TextColored(sc,st); local p5ps=p4s+sw+sp; imgui.SameLine(p5ps); 
            local ping=p.Ping or 0; local pc=get_ping_color(ping); 
            local dev_str = state.player_devices[p.id] and (" ["..state.player_devices[p.id].."]") or ""; local pt=string.format("%d ms%s",math.floor(ping), dev_str); imgui.TextColored(pc,pt)
            local dev_data = state.player_devices[p.id]
            local dev_str = ""
            if dev_data then
                dev_str = " [" .. dev_data.type .. "]"
            end
            local pt=string.format("%d ms%s",math.floor(ping), dev_str); imgui.TextColored(pc,pt)
        end
    end
...
function imgui.OnDrawFrame()
    if state.window_open.v then
        ...
        imgui.Begin("Painel Admin [F12] - v8.9.41", state.window_open)
        ...
        if imgui.BeginPopupModal("AjudaPainel", nil, imgui.WindowFlags.AlwaysAutoResize) then
            imgui.TextColored(IMAGE_YELLOW, "Ajuda - Painel Admin")
            imgui.Separator()
            imgui.Text("Este painel auxilia na administracao e informacoes do servidor.")
            imgui.Spacing()
            imgui.TextColored(IMAGE_GREEN, "Funcionalidades por Aba:")
            imgui.BulletText("Novatos/Online: Lista jogadores. Clique com Botao Direito no nome para opcoes (Espiar, Ir, IP).")
            imgui.BulletText("Informacoes: Listas de IDs (Veiculos, Skins, Armas). Duplo clique para aplicar.")
            imgui.BulletText("Locais: Teleportes. Use 'Salvar Atual' para gravar locais personalizados.")
            imgui.BulletText("Comandos: Ferramentas como Extrator de IP e atalhos.")
            imgui.BulletText("Config: Ajuste de tema, senha admin, ESP (Wallhack) e distancia.")
            imgui.Spacing()
            imgui.Text("Dica: Use o filtro na aba Online para ver apenas PC ou Mobile.")
            imgui.Separator()
            if imgui.Button("Fechar", imgui.ImVec2(400, 0)) then imgui.CloseCurrentPopup() end
            imgui.EndPopup()
        end
            ...
            for _, handle in ipairs(getAllChars()) do
                if doesCharExist(handle) and handle ~= PLAYER_PED then
                    ...
                    if res and sampIsPlayerConnected(id) then
                        local skip = false
                        if state.spectate_id ~= -1 and id == state.spectate_id then skip = true end
                        if not skip and state.spectate_vehicle_id ~= -1 and isCharInAnyCar(handle) then
                            local car = getCarCharIsUsing(handle)
                            local res, carId = sampGetVehicleIdByCarHandle(car)
                            if res and carId == state.spectate_vehicle_id then skip = true end
                        end
                        if not skip then
                            local x, y, z = getCharCoordinates(handle)
                            if isPointOnScreen(x, y, z, 0.0) then
                                local dist = getDistanceBetweenCoords3d(myX, myY, myZ, x, y, z)
                                local checkVeh = true; if isCharInAnyCar(handle) then checkVeh = false end
                                local camX, camY, camZ = getActiveCameraCoordinates()
                                local visible = isLineOfSightClear(camX, camY, camZ, x, y, z + 0.7, true, checkVeh, false, true, false)
                                if dist < cfg.main.esp_dist and (not visible or dist > 65) then
                                    local headX, headY = convert3DCoordsToScreen(x, y, z + 2.6)
                                    if headX and headY then
                                        local nick = sampGetPlayerNickname(id) or "Unknown"; local hp = getCharHealth(handle); local armor = getCharArmour(handle)
                                        if hp > 100 then hp = 100 end; if hp < 0 then hp = 0 end; if armor > 100 then armor = 100 end
                                        local pColor = sampGetPlayerColor(id); if pColor == 0 then pColor = 0xFFFFFFFF end; pColor = bit.bor(pColor, 0xFF000000)
                                        local text = string.format("%s (%d)", nick, id); local textLen = renderGetFontDrawTextLength(font_esp, text)
                                        local textX = headX - (textLen / 2); local textY = headY
                                        renderFontDrawText(font_esp, text, textX, textY, pColor)
                                        ...
                                        if armor > 0 then local armY = nextY; renderDrawBox(barX - 1, armY - 1, barW + 2, barH + 2, 0xFF000000); local armW = barW * (armor / 100); renderDrawBox(barX, armY, armW, barH, 0xFFF5F5F5); nextY = armY + barH + 2 end
                                        local wep = getCurrentCharWeapon(handle)
                                        if wep and wep > 0 and esp_weapon_names[wep] then local wname = esp_weapon_names[wep]; local wLen = renderGetFontDrawTextLength(font_esp, wname); local wY = nextY; renderFontDrawText(font_esp, wname, headX - (wLen / 2), wY, 0xFFFFFFFF) end
                                    end
                                end
                            end
                        end
