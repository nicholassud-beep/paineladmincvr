...
script_version("1.0.44-AutoUpdate")
...
-- =========================================================
local UPDATE_URL = "https://raw.githubusercontent.com/nicholassud-beep/paineladmincvr/main/PainelInfoHelper.lua" -- COLOQUE AQUI O LINK RAW DO SEU SCRIPT (Ex: https://raw.githubusercontent.com/.../PainelInfoHelper.lua)
...
function sampev.onTogglePlayerSpectating(toggle)
    if toggle then
        ...
        if state.stop_spec_requested then
...
function imgui.OnDrawFrame()
    if state.window_open.v then
        ...
        imgui.Begin("Painel Helper [F12] - v1.0.44", state.window_open)
                                ...
                                if sampIsPlayerConnected(i) then
                                    ...
                                    if nick then
                                        ...
                                        for attempt = 1, 2 do
                                            ...
                                            -- Espera a resposta ou timeout (300ms)
                                            local timeout = 0
                                            while not state.scan_response_received and timeout < 300 do
                                                wait(10)
                                                timeout = timeout + 10
                                                -- Se ja recebeu msg (IP) e passou 50ms, assume que acabou (para PC implicito ser rapido)
                                                if state.scan_message_count > 0 and timeout > 100 then break end
                                            end
        ...
        elseif state.active_tab == 11 then
            ...
            imgui.Checkbox("Ativar Wallhack (ESP)", state.esp_active_check)
        ...
        if imgui.BeginPopupModal("AjudaPainel", nil, imgui.WindowFlags.AlwaysAutoResize) then
            imgui.TextColored(IMAGE_YELLOW, "Ajuda - Painel Info Helper")
            imgui.Separator()
